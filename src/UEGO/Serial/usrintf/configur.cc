	...
	while( !ValueRead )
	{
		...
		switch( puffer[0] )
		{
			case 'q': // cancel config
				message((char*)"Configuration cancelled.",
							MSG_INFORMATION);
...
void	Configure::ReadParFile( FILE* parfile ) {
	...
	{
		message( (char*)"Bad parameter file (length).",MSG_ERROR);
		return;
	};
	...
	if( ini->param == NULL )
	{
		message( (char*)"No memory in configuration.",MSG_ERROR);
		return;
	};
	...
	if( i < ini->paramnum )
	{
		message((char*)"Bad parameter file (parameters).",MSG_ERROR);
		return;
	};
...
void	Configure::GetParameterVector() {
	...
	parfile = GetOpenFile((char*) "Parameter text file? (<no file>):", (char*)PARS_HE );
	...
	do newparnum = (long)GetValue(prompt, (char*)PARNUM_HE, (double)ini->paramnum);
	...
	for( i=0; !Fail() && i < newparnum; ++i )
	{
		sprintf( prompt, "%ld. constant parameter (%lf): ",
			i+1, ini->param[i] );
		ini->param[i] = GetValue( prompt, (char*)"", ini->param[i] );
	};
};
...
void	Configure::GetBoundVectors( long newdim ) {
	...
	for( i=0; !Fail() && i < newdim; ++i )
	{
		sprintf( prompt, "%ld. component of lower bound (%lf): ",
			i+1, ini->lowb[i] );
		ini->lowb[i] = GetValue( prompt, (char*)"", ini->lowb[i] );
	};
	for( i=0; !Fail() && i < newdim; ++i )
	{
		sprintf( prompt, "%ld. component of upper bound (%lf): ",
			i+1, ini->upb[i] );
		do d = GetValue( prompt, (char*)"", ini->upb[i] );
...
// on this interval
	...
	if( Diff() > 0 )
	{
		message((char*)"No solution, parameters not set.",MSG_ERROR);
		FailFlag = 1==1;
		return;
	};
...
void	Configure::ReAllocVectors( long levels ) {
	...
	{
		message((char*)"No memory for storing parameters.",MSG_ERROR);
		return;
	};
...
WARN:	if( ini->levels == 1 )
	{
		message( (char*)"levels set to 1, special uego things won't be used!",
							MSG_INFORMATION);
		return;
	};
...
void	Configure::DisplayConfig() {
	...
	ini->seed = (unsigned long)GetValue(prompt, (char*)SEED_HE, (double)ini->seed);

	// --- objective function ------------------------------------------
	sprintf( prompt, "%s (%ld): ", TYPE_T, ini->type );
	do l = (long) GetValue( prompt, (char*)TYPE_HE, (double)ini->type );
	while( !Fail() && !VALID_TYPE(l) );
	ini->type = l;

	sprintf( prompt, "%s (%ld): ", FNUM_T, ini->fnum );
	ini->fnum  = (long) GetValue( prompt, (char*)FNUM_HE, (double)ini->fnum );

	sprintf( prompt, "%s (%ld): ", DIM_T, ini->dimension );
	do l = (long) GetValue( prompt, (char*)DIM_HE, (double)ini->dimension );
	while( !Fail() && l < 1 );
	...
	ini->maxevals = (unsigned long) GetValue( prompt, (char*)MAXEV_HE,
							(double)ini->maxevals );

	sprintf( prompt, "%s (%ld): ", MAXSPEC_T, ini->maxspecnum );
	do l = (long) GetValue( prompt, (char*)MAXSPEC_HE, (double)ini->maxspecnum );
	while( !Fail() && l < 1 );
	ini->maxspecnum = l;

	sprintf( prompt, "%s (%lg): ", THR_T, ini->threshold );
	do d = GetValue( prompt, (char*)THR_HE, ini->threshold );
	while( !Fail() && d <= 0.0 );
	ini->threshold = d;

	sprintf( prompt, "%s (%ld): ", LEVELS_T, (long)ini->levels );
	do l = (long) GetValue( prompt, (char*)LEVELS_HE, (double)ini->levels );
	while( !Fail() && l < 1 );
	ini->levels = (short)l;
	...
	do d = GetValue( prompt, (char*)LAST_R_HE, last_r );
	while( !Fail() && (d <= 0.0 || d >= ini->prototype->Diameter(ini)));
	last_r = d;

	sprintf( prompt, "Parameter to ignore (2): " );
	do l = (long) GetValue( prompt, (char*)MISS_HE, 2.0 );
...
// the parameter order is fixed in command line
	...
	else message((char*)"Command line configuration failed.",MSG_ERROR);
};
...
void	Configure::Config( char* ininame, int argc, char** argv ) {
	...
	{
		message((char*)"Could not create default settings.",MSG_ERROR);
		return;
	};
	...
	if( !Fail() )
		message( (char*)"Configuration successful.", MSG_INFORMATION );
	delete ini;
};
