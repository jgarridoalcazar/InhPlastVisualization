...
namespace mynest
{
  ...
  inline
  nest::port iaf_cond_exp_sto_ip::check_connection(nest::Connection& c, nest::port receptor_type)
  {
	nest::SpikeEvent e;
    e.set_sender(*this);
    c.check_event(e);
  ...
  inline
  nest::port iaf_cond_exp_sto_ip::connect_sender(nest::SpikeEvent&, nest::port receptor_type)
  {
    if (receptor_type != 0)
  ...
  inline
  nest::port iaf_cond_exp_sto_ip::connect_sender(nest::CurrentEvent&, nest::port receptor_type)
  {
    if (receptor_type != 0)
      throw nest::UnknownReceptorType(receptor_type, get_name());
    return 0;
  ...
  inline
  nest::port iaf_cond_exp_sto_ip::connect_sender(nest::DataLoggingRequest& dlr,
