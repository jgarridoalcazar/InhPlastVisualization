...
class HitTopAnalysis(Analysis.Analysis):
    '''
    ...
    def __init__(self,**kwargs):
        ...
        @param number_of_cells: Number of cells (n) to consider in the top. Optional parameter. If not specified, all the cells in cell_index will be used.
        '''
        ...
        # Get index parameter
        if ('number_of_cells' in kwargs):
            self.number_of_cells = kwargs.pop('number_of_cells',None)
        else:
            self.number_of_cells = len(self.cell_index)
        ...
        super(HitTopAnalysis, self).__init__(**kwargs)
    ...
    def initialize(self):
        ...
        super(HitTopAnalysis, self).initialize()
    ...
    #             pattern_prob = numpy.sum(self.bin_is_pattern[:,init_bin:end_bin],axis=1) / float(end_bin-init_bin)
        ...
        ind = numpy.where(numpy.in1d(selected_pattern,self.pattern_index))[0]
        ...
        ind_top = numpy.argsort(hit_fa_diff_per_cell)[-self.number_of_cells:]
        self.top_n_average = numpy.average(hit_fa_diff_per_cell[ind_top])

        # Check if any of the patterns are not represented        
        selected_pattern_top = selected_pattern[ind[ind_top]]
        unique_pattern = numpy.unique(selected_pattern_top)
        if (unique_pattern.size<len(self.pattern_index)):
            logger.debug(str('Some patterns are not represented in the top cells'))
            self.top_n_average = 0.0
            
        logger.info(str('Hit-false alarm of top-n index:')+str(self.top_n_average))
