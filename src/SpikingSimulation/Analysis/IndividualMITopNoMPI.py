...
class IndividualMITop(Analysis.Analysis):
    '''
    This class defines the calculation of the average mutual information (average of individual neurons)
    between a single pattern and the cellular activity. The neural population is split in P subsets of N/P
    neurons (where P is the number of selected patterns and N is the number of neurons). Each subset is composed
    of the top N/P neurons with the highest MI between the nueron activity and one of the pattern. The estimator
    calculates the average MI of those neurons regarding those patterns.
    '''
    ...
    def __init__(self,**kwargs):
        ...
        super(IndividualMITop, self).__init__(**kwargs)
    ...
    def initialize(self):
        ...
        super(IndividualMITop, self).initialize()
    ...
    #             pattern_prob = numpy.sum(self.bin_is_pattern[:,init_bin:end_bin],axis=1) / float(end_bin-init_bin)
        ...
        logger.debug('MI matrix: %s', MI_Matrix)

        sortedMI = numpy.argsort(MI_Matrix, axis=1)
        num_neurons = MI_Matrix.shape[1]
        num_patterns = MI_Matrix.shape[0]
        sumMI = 0
        for pat, row in enumerate(sortedMI):
            sumMI = sumMI + numpy.sum(MI_Matrix[pat, row[-num_neurons/num_patterns:]])
        avMI = sumMI / ((num_neurons / num_patterns) * num_patterns)

        avPat = numpy.average(pat_entropy)

        logger.debug('Average MI of top-cells to each pattern: %s', avMI)

        self.mutual_information, self.max_mutual_information = avMI,avPat
