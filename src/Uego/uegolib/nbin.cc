...
// 'long' is assumed to be at least 32 bit long
	...
	if( FailFlag ) message("No memory for BinaryElement.",MSG_ERROR);
};
...
// random bits
	...
	{
		message( "Error creating empty element in BinaryElement::RandNew",MSG_ERROR);
...
// currently mutation probability (determined by MutJump) is fixed
	...
	if( result == NULL || result->Fail() )
	{
		message( "Error creating empty element in BinaryElement::MutateNew",MSG_ERROR);
		return (SearchSpElement*)NULL;
	};
...
// uniform crossover
	...
	if( result == NULL || result->Fail() )
	{
		message( "Error creating empty element in BinaryElement::BetweenNew",MSG_ERROR);
		return (SearchSpElement*)NULL;
	};
...
void	BinaryElement::Save( FILE* stream ) {
	...
	if( Fail() ) message("Error writing BinaryElement.",MSG_ERROR);
};
