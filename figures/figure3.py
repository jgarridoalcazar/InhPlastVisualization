...
create_figure_at_time(17.)
matplotlib.pylab.savefig('figure3A.svg', format='svg', dpi=1200) 
matplotlib.pylab.show()

create_figure_at_time(1500.)
matplotlib.pylab.savefig('figure3B.svg', format='svg', dpi=1200) 
matplotlib.pylab.show()
...
def create_figure_weights_at_time(time):
    ...
    import SpikingSimulation.Visualization.AxesWeightEvolutionLine as AxesWeightEvolutionLine
    
    
                
    figureA = SimulFigure.SimulFigure(simulation = simulation, numRows=2,numColumns=3,figsize=[23,14],dpi=80)
    ...
    figureA.add_subplot(fig_position=2,axes_type=AxesActivationFiringOffset.AxesActivationFiringOffset,
                        axes_parameters= {'data_provider':simulation.cerebellum,
                                          ...
                                          'layer':'mflayer',
                                          'visible_data_only':True})
    figureA.add_subplot(fig_position=4,axes_type=AxesWeightActivationPlot.AxesWeightActivationPlot,
                            axes_parameters= {'data_provider':simulation.cerebellum,
                                              'pattern_provider': simulation.pattern_generator,
                                              'layer':'mfgocsynapsis',
                                              'target_indexes' : [0],
    ...
    figureA.add_subplot(fig_position=5,axes_type=AxesWeightActivationPlot.AxesWeightActivationPlot,
                            axes_parameters= {'data_provider':simulation.cerebellum,
                                              'pattern_provider': simulation.pattern_generator,
                                              'layer':'mfgocsynapsis',
                                              'target_indexes' : [1],
                                              'show_legend':False,
                                              'y_window_lim': [-0.01e-9,1.0e-9]}) 
    figureA.add_subplot(fig_position=6,axes_type=AxesWeightEvolutionLine.AxesWeightEvolutionLine,
                            axes_parameters= {'data_provider':simulation.cerebellum,
...
matplotlib.pylab.savefig('figure3C.svg', format='svg', dpi=1200) 
matplotlib.pylab.show()